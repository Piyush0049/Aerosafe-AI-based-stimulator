  ...
  return (
    <group>
      <mesh ref={ref} castShadow receiveShadow onClick={(e) => { e.stopPropagation(); setSelected(uav.id); }}>
        ...
        <meshStandardMaterial color={color} emissive={color} emissiveIntensity={0.35} metalness={0.2} roughness={0.35} />
        {/* Label */}
        <Html center distanceFactor={8} position={[0, uav.radius + 6, 0]}>
          <div style={{ fontFamily: 'Poppins, sans-serif' }} className="px-2 py-0.5 rounded-md text-xs bg-black/70 text-white border border-white/10 whitespace-nowrap">
            {uav.id} • {Math.round(uav.position.z)} m
          </div>
        </Html>
      </mesh>
      {/* Velocity vector */}
      <Line points={[[uav.position.x, uav.position.z, uav.position.y], velocityEnd.current]} color={color} lineWidth={2} transparent opacity={0.8} />
